...
import { canSendContact, sendContact } from "../../content/sendContact";
    ...
    <div className="bg-zinc-900 text-white min-h-[70vh]">
      <div className="max-w-3xl mx-auto px-4 py-16 sm:py-20">
        ...
        <form className="bg-zinc-800 p-5 sm:p-8 space-y-5 text-left" onSubmit={onSubmit}>
          ...
          <label className="block">
            <span className="block mb-1">{contact.messageLabel}</span>
            <textarea
              name="message"
              required
              rows={5}
          ...
          {!ready && status !== "sent" && <p className="text-sm text-slate-400">{contact.blockedHint}</p>}
          {status === "sent" && <p className="text-orange-300">{contact.success}</p>}
